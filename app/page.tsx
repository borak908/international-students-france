...
export default function HomePage() {
  return (
    <>
      {/* ── Hero ── */}
      <section className="relative bg-gradient-to-br from-[#12312A] via-[#194035] to-[#22513F] text-white overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-20 sm:py-28 text-center">
          <span className="inline-block text-xs font-semibold tracking-widest text-[#8ECFC0] uppercase mb-4">
            The complete guide
          </span>
          ...
          <p className="text-[#B8E0D5] text-base sm:text-lg max-w-2xl mx-auto mb-10 leading-relaxed">
            Compare cities by housing cost, universities, transport, visa rules, and student life.
            Real data. No fluff.
          </p>
          <a
            href="#cities"
            className="inline-flex items-center gap-2 bg-white text-[#12312A] font-semibold px-6 py-3 rounded-full hover:bg-[#E8F6F2] transition-colors text-sm shadow-lg"
          >
            Explore cities
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {cityData.map((city) => (
            <Link
              ...
              className="group bg-white rounded-2xl border border-stone-200 hover:border-[#8ECFC0] hover:shadow-lg transition-all duration-200 overflow-hidden"
            >
              {/* Card top colour bar — French tricolour for all */}
              <div className="h-1.5 bg-gradient-to-r from-[#5580C0] via-white to-[#B86040]" />

              <div className="p-6">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <h3 className="text-xl font-bold text-stone-800 group-hover:text-[#1E7A5E] transition-colors">
                      {city.name}
                    </h3>
                    <p className="text-xs text-stone-400 mt-0.5">{city.region}, France</p>
                  </div>
                  <svg
                    className="w-5 h-5 text-stone-300 group-hover:text-[#1E7A5E] transition-colors mt-0.5"
                ...
                <div className="mt-5 flex flex-wrap gap-2">
                  {city.universities.grandesEcoles.slice(0, 3).map((school) => (
                    <span
                      key={school}
                      className="bg-[#E8F6F2] text-[#1E7A5E] text-xs font-medium px-2.5 py-1 rounded-full"
                    >
                      {school}
                    </span>
                  ))}
              ...
              <tr className="border-b border-stone-200">
                ...
                {cityData.map((city) => (
                  <th key={city.slug} className="px-3 py-4 text-center font-bold text-stone-800 whitespace-nowrap min-w-[110px]">
                    <Link href={`/cities/${city.slug}`} className="hover:text-[#1E7A5E] transition-colors">
                      {city.name}
                    </Link>
                  </th>
                ))}
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 pb-16">
        <div className="bg-gradient-to-r from-[#E8F6F2] to-[#F0F8F5] border border-[#B8DACF] rounded-2xl p-6 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center gap-4">
